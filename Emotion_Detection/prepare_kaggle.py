...
def split_data(source_dir=INPUT_DIR, output_dir=OUTPUT_DIR, train_ratio=0.7, test_ratio=0.15, valid_ratio=0.15):
    """
    Split emotion data into train, test, and validation sets for Kaggle.
    
    Args:
        source_dir: Directory containing emotion class folders (Kaggle input)
        output_dir: Directory where output splits will be created (Kaggle working)
    ...
    emotion_classes = [d.name for d in source_path.iterdir() 
                      if d.is_dir() and d.name not in ['train', 'test', 'valid']]
    ...
    for emotion in emotion_classes:
        ...
        # Copy files to respective directories (use copy instead of move for Kaggle)
        for file in train_files:
            dest = output_path / 'train' / emotion / file.name
            shutil.copy2(str(file), str(dest))
        
        for file in test_files:
            dest = output_path / 'test' / emotion / file.name
            shutil.copy2(str(file), str(dest))
        
        for file in valid_files:
            dest = output_path / 'valid' / emotion / file.name
            shutil.copy2(str(file), str(dest))
    ...
    return stats, emotion_classes, output_path

def create_pie_chart(stats, emotion_classes, save_path=None):
    """
    ...
    Args:
        ...
        save_path: Path to save the pie chart (defaults to Kaggle working/results/)
    """
...
def main():
    """Main function to prepare data for Kaggle."""
    print("="*60)
    print("KAGGLE DATA PREPARATION")
    print("="*60)
    ...
    # Split data
    stats, emotion_classes, output_path = split_data()
